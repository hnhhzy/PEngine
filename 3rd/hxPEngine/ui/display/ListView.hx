...
class ListView extends ScrollView implements IListView {
	...
	override function onInit() {
		...
		this.addEventListener(hxPEngine.ui.events.Event.CLICK, onItemRendererClick);
	}

	private function onItemRendererClick(e:hxPEngine.ui.events.Event):Void {
		...
		if (this.selectedItem != value) {
			...
			this.updateData();
			// 发生变化时触发
			this.dispatchEvent(new hxPEngine.ui.events.Event(hxPEngine.ui.events.Event.CHANGE));
		}
	}
