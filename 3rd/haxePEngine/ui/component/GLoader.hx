...
import haxePEngine.ui.component.base.events.IOErrorEvent;
...
class GLoader extends GObject implements IColorGear implements IAnimationGear
{
    ...
    private function set_playing(value:Bool):Bool
    {
        ...
        {
            _playing = value;
            if (Std.isOfType(_content, haxePEngine.ui.display.MovieClip))
                cast(_content, haxePEngine.ui.display.MovieClip).playing = value;
            else if (Std.isOfType(_content, haxePEngine.ui.component.base.display.MovieClip))
                cast(_content, haxePEngine.ui.component.base.display.MovieClip).stop();
            updateGear(5);
        }
        return value;
    }
    ...
    private function set_frame(value:Int):Int
    {
        if (_frame != value)
        {
            _frame = value;
            if (Std.isOfType(_content, haxePEngine.ui.display.MovieClip))
                cast(_content, haxePEngine.ui.display.MovieClip).currentFrame = value;
            else if (Std.isOfType(_content, haxePEngine.ui.component.base.display.MovieClip))
            {
                if (_playing)
                    cast(_content, haxePEngine.ui.component.base.display.MovieClip).gotoAndPlay(_frame + 1);
                else
                    cast(_content, haxePEngine.ui.component.base.display.MovieClip).gotoAndStop(_frame + 1);
            }
            updateGear(5);
        }
        return value;
    ...
    private function __movieClipLoaded(pi:PackageItem):Void
    {
        ...
        if (!(Std.isOfType(_content, haxePEngine.ui.display.MovieClip)))
        {
            _content = new haxePEngine.ui.display.MovieClip();
            _container.addChild(_content);
        }
        else
            _container.addChild(_content);
        ...
        cast(_content, haxePEngine.ui.display.MovieClip).interval = Std.int(pi.interval);
        cast(_content, haxePEngine.ui.display.MovieClip).frames = pi.frames;
        cast(_content, haxePEngine.ui.display.MovieClip).repeatDelay = Std.int(pi.repeatDelay);
        cast(_content, haxePEngine.ui.display.MovieClip).swing = pi.swing;
        cast(_content, haxePEngine.ui.display.MovieClip).boundsRect = new Rectangle(0, 0, _contentSourceWidth, _contentSourceHeight);
    ...
    private function __swfLoaded(content:DisplayObject):Void
    {
        ...
        if (_content != null && (Std.isOfType(_content, haxePEngine.ui.component.base.display.MovieClip)))
        {
            if (_playing)
                cast(_content, haxePEngine.ui.component.base.display.MovieClip).gotoAndPlay(_frame + 1);
            else
