package haxePEngine.ui.component.hbase.base.text.glyphme;
...
import haxePEngine.ui.component.hbase.base.text.glyphme.GlyphMe;
 ...
 * to look up glyphs that it couldn't find in the previous TrueTypeFontInfo (when generating atlas). */
 class TrueTypeFont extends h2d.Font {
	...
	public var scaleMode:TrueTypeFontScaleMode = TrueTypeFontScaleMode.Ascent;
	...
	public function new(infos:Array<TrueTypeFontInfo>, sizeInPixels:Int, alphaCutOff:Float, smoothing:Float) {
		...
		//trace((ascent + lineGap) * sizeInPixels * scale );
		this.lineHeight = sizeInPixels * 1.25;
		//trace(super.lineHeight);
		//super.lineHeight = (Std.int(super.lineHeight) / 0.7 - super.lineHeight) / 2 + super.lineHeight;
		...
		function tryToPack(index:Int, originX:Int, originY:Int, bounds:Int) {
			...
			while (index < length) {
				...
				if (y + height > originY + bounds) {
					...
					//atlas.blit(x, y + Math.round(Std.int(padding)), brush, 0, 0, width, height);
					atlas.blit(x, y + Math.round(padding), brush, 0, 0, width, height);
	...
	function generateGlyph(code:Int, info:TrueTypeFontInfo, p:TrueTypeFontGenerationParameters):TrueTypeFontGlyphInfo {
		...
		p.padding = Math.round(p.fontHeightInPixels * 1.25 - p.fontHeightInPixels);
		//trace(p.padding);
